...
export class Xml11Parser extends BaseXmlParser {
  public get loaiHoSo(): string {
    return 'XML11';
  }

  public parse(xmlContent: string): Xml11Model {
    const model: Xml11Model = {};

    if (!this.load(xmlContent)) return model;
    if (!this.isRoot('CHI_TIEU_DU_LIEU_GIAY_CHUNG_NHAN_NGHI_VIEC_HUONG_BAO_HIEM_XA_HOI')) return model;
