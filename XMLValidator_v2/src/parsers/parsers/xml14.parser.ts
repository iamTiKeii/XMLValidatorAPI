...
export class Xml14Parser extends BaseXmlParser {
  public get loaiHoSo(): string {
    return 'XML14';
  }

  public parse(xmlContent: string): Xml14Model {
    const model: Xml14Model = {};

    if (!this.load(xmlContent)) return model;
    if (!this.isRoot('CHI_TIEU_GIAYHEN_KHAMLAI')) return model;
