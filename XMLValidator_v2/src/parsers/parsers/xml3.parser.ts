...
export class Xml3Parser extends BaseXmlParser {
  public get loaiHoSo(): string {
    return 'XML3';
  }

  public parse(xmlContent: string): Xml3Model[] {
    const result: Xml3Model[] = [];

    if (!this.load(xmlContent)) return result;
    if (!this.isRoot('CHITIEU_CHITIET_DVKT_VTYT')) return result;
