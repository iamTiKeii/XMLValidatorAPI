...
export class Xml7Parser extends BaseXmlParser {
  public get loaiHoSo(): string {
    return 'XML7';
  }

  public parse(xmlContent: string): Xml7Model {
    const model: Xml7Model = {};

    if (!this.load(xmlContent)) return model;
    if (!this.isRoot('CHI_TIEU_DU_LIEU_GIAY_RA_VIEN')) return model;
