...
export class Xml6Parser extends BaseXmlParser {
  public get loaiHoSo(): string {
    return 'XML6';
  }

  public parse(xmlContent: string): Xml6Model[] {
    const result: Xml6Model[] = [];

    if (!this.load(xmlContent)) return result;
    if (!this.isRoot('CHI_TIEU_HO_SO_BENH_AN_CHAM_SOC_VA_DIEU_TRI_HIV_AIDS')) return result;
