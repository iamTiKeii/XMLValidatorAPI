...
export class Xml13Parser extends BaseXmlParser {
  public get loaiHoSo(): string {
    return 'XML13';
  }

  public parse(xmlContent: string): Xml13Model {
    const model: Xml13Model = {};

    if (!this.load(xmlContent)) return model;
    if (!this.isRoot('CHI_TIEU_GIAYCHUYENTUYEN')) return model;
